...
public class SwerveModule extends SubsystemBase {
    ...
    public void velocityDrive(Vector speedVector, double turnRate, double navxOffset) {
        ...
        if(Math.abs(speedVector.getI()) < 0.0001 && Math.abs(speedVector.getJ()) < 0.0001 && Math.abs(turnRate) < 0.01) {
            ...
            //Case three changes the angle to 180 degrees from target moving counterclockwise, but will reverse the way that the wheel spins(-velocity)
            double caseThreeAngle = adjustedAngle + Math.toRadians(180);
            
            //Case four changes the angle to 180 degrees from target moving clockwise, but will reverse the way that the wheel spins(-velocity)
